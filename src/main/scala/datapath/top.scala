...
class Top extends Module{
    ...
    reg_file.rd.rd:=Mem_wb.io.rd_out
    reg_file.io.rs1:=ID.io.rs1.Ins_out
    ...
    alu.io.in1:= MuxCase ( 0.S, Array (
                (ID.io.operandA_out=== "b00". U ||ID.io.operandA_out=== "b11". U) -> reg_file.io.rd1 , //reg aleardy exist in rd1 so need to add
                (ID.io.operandA_out=== "b01". U) -> ID.io.pc4_out.asSInt,
                (ID.io.operandA_out=== "b10". U) -> ID.io.pc_out.asSInt )
             )
    ...
    //EX
    alu.io.in1:=ID.io.rs1Ins_out
    when(ID.io.operandBsel_out === "b1".U) {
    alu.io.in2 := immreg
    } .otherwise {
    alu.io.in2 := ID.io.rs2Ins_out
    }
    ...
    // Operand A
	when(alu_controler.io.op_a === "b01".U){
		ID.io.operandA_in := (IF.io.pc_out).asSInt
	}.elsewhen(alu_controler.io.op_a === "b10".U){
		ID.io.operandA_in := (IF.io.pc4_out).asSInt 
	}.otherwise{
		ID.io.operandA_in := regFile.io.rd1
	}	

	//Operand B
	when(alu_controler.io.extend_sel === "b00".U & alu_controler.io.op_b === 1.U){
		ID.io.operandB_in := imm.io.i_imm
	}.elsewhen(alu_controler.io.extend_sel === "b01".U & alu_controler.io.op_b === 1.U){
		ID.io.operandB_in := imm.io.u_imm
	}.elsewhen(alu_controler.io.extend_sel === "b10".U & alu_controler.io.op_b === 1.U){
		ID.io.operandB_in := imm.io.s_imm
	}.otherwise{
		ID.io.operandB_in := regFile.io.rd2
	}
